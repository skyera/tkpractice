...
class PlotFrame(ttk.Frame):
    ...
    def make_axes(self):
        ...
        self.canvas.create_text(225, 20, text='A plot', font=self.plot_font, fill='brown')

    def make_x_axis_labels(self):
        for i in range(11):
            x = 100 + i*30
            self.canvas.create_line(x, 250, x, 245, width=2)
            self.canvas.create_text(x,254, text='{}'.format((10*i)),
                    anchor=tk.N, font=self.plot_font)

    def make_y_axis_labels(self):
        for i in range(1,6):
            y = 250 - (i*40)
            self.canvas.create_line(100, y, 105, y, width=2)
            self.canvas.create_text(96, y, text='{}'.format((i*50.0)),
                    anchor=tk.E, font=self.plot_font)
    ...
    def make_data_points(self):
        ...
        for p0, p1 in data:
            x = 100 + 3 * p0
            y = 250 - (4*p1)/5
            item = self.canvas.create_oval(x-6, y-6, x+6, y+6, width=1,
